...
class PerformaSheetController extends Controller
{
            ...
            foreach ($performaSheets as $performa) {
                ...
                if ($request->status === 'rejected') {
                    ...
                    if ($oldStatus === 'approved') {
                        ...
                        if ($project && !empty($originalData['time'])) {
                            try {
                                $time = Carbon::createFromFormat('H:i', trim($originalData['time']));
                                $hours = $time->hour + ($time->minute / 60);

                                $project->project_used_hours = max(
                                    0,
                                    $project->project_used_hours - $hours
                                );
                                $project->save();
                            } catch (\Exception $e) {
                            }
                ...
                try {
                    $time = Carbon::createFromFormat('H:i', trim($originalData['time']));
                    $submittedHours = $time->hour + ($time->minute / 60);
                } catch (\Exception $e) {
                    $results[] = [
                        'performa_id' => $performa->id,
                        'status' => 'failed',
                ...
                if (!$project->project_tracking) {
                    $project->project_used_hours += $submittedHours;
                    $project->save();
                }
                ...
                ActivityService::log([
                    'project_id' => $project->id,
                    'type' => 'activity',
                    'description' => 'Performa approved by ' . auth()->user()->name,
                ]);
            }
        });
    ...
    public function getAllPendingPerformaSheets(Request $request)
    {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Pending sheets fetched successfully',
            'data' => $paginated->values(),
            'pagination' => [
                'current_page' => $paginated->currentPage(),
                'last_page' => $paginated->lastPage(),
            ...
            foreach ($finalData as $team) {
                ...
                foreach ($team['teamMembers'] as $member) {
                    $members[] = [
                        "user_id" => $member['user_id'],
                        "name" => $member['name'],
                        "billable" => $toTime($member['billable']),
                        "inhouse" => $toTime($member['inhouse']),
                        "no_work" => $toTime($member['no_work']),
                ...
                $response[] = [
                    "teamName" => $team['teamName'],
                    "expectedHours" => $toTime($team['expectedMinutes']),
                    "billableHours" => $toTime($team['billableMinutes']),
                    "inhouseHours" => $toTime($team['inhouseMinutes']),
    ...
    public function getPendingPerformaSheetsByProjectMasterId(Request $request)
    {
        ...
        $performaSheets = PerformaSheet::whereIn('user_id', $userIds)
            ...
            ->filter(function ($sheet) use ($search, $searchBy, $projectMap) {
                ...
                $projectName = $projectId && isset($projectMap[$projectId])
                ...
                if ($searchBy === 'name') {
                    ...
                    return stripos($projectName, $search) !== false;
                } elseif ($searchBy === 'activity_type') {
                    return stripos($activityType, $search) !== false;
                }

                return
                    stripos($userName, $search) !== false ||
                    stripos($projectName, $search) !== false ||
                    stripos($activityType, $search) !== false;
            })
        ...
        foreach ($performaSheets as $sheet) {
            ...
            $dataArray = is_string($sheet->data)
                ? json_decode($sheet->data, true)
                : $sheet->data;
            ...
            $dataArray['project_name'] = $projectMap[$dataArray['project_id']] ?? null;
            $dataArray['status'] = $sheet->status;
            $dataArray['id'] = $sheet->id;
            $dataArray['created_at'] = optional($sheet->created_at)->format('Y-m-d H:i:s');
            $dataArray['updated_at'] = optional($sheet->updated_at)->format('Y-m-d H:i:s');
        ...
        return response()->json([
            'success' => true,
            'message' => 'Pending Performa Sheets fetched successfully',
            'data' => $paginated->values(),
            'pagination' => [
                'current_page' => $paginated->currentPage(),
                'last_page' => $paginated->lastPage(),
